...
function clearFileArtifacts(graphId, fileId, fileName = '') {
...
router.post('/:graphId', async (req, res) => {
  try {
    ...
    const graphStats = rebuildCanonicalGraph(graphId, allRows, run)
    run(
      `
    ...
    res.json({
      success: true,
      id,
      paragraphCount: records.counts.paragraphCount,
      segmentCount: records.counts.segmentCount,
      entityMentionCount: records.counts.entityMentionCount,
      eventMentionCount: records.counts.eventMentionCount,
      relationMentionCount: records.counts.relationMentionCount,
      graph: graphStats
    })
  } catch (error) {
    res.status(500).json({ error: error.message })
...
router.delete('/:graphId/detail/:fileId', async (req, res) => {
  try {
    ...
    res.json({ success: true, graph: graphStats })
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
})
...
router.delete('/detail/:fileId', async (req, res) => {
  try {
    const file = getRow('SELECT id, graphId, fileName FROM files WHERE id = ?', [req.params.fileId])
    if (!file) return res.status(404).json({ error: 'File not found' })

    clearFileArtifacts(file.graphId, file.id, file.fileName)
    run('DELETE FROM files WHERE id = ?', [file.id])
    run('DELETE FROM import_history WHERE id = ? AND graphId = ?', [file.id, file.graphId])

    const graphStats = rebuildCanonicalGraph(file.graphId, allRows, run)
    saveToDisk()
    await rebuildGraphDb(file.graphId)
